...
TreeGrid = function(_config){
	...
	this.show = function(){
		...
		__root = jQueryE("#"+_config.renderTo);
		__root.append(s);
	...
	init = function(){
		...
		if((_config.hoverRowBackground || "false") == "true"){
			__root.find("tr").hover(
				function(){
					if(jQueryE(this).attr("class") && jQueryE(this).attr("class") == "header") return;
					jQueryE(this).addClass("row_hover");
				},
				function(){
					jQueryE(this).removeClass("row_hover");
				}
			);
		}
		...
		__root.find("tr").bind("click", function(){
			...
			jQueryE(this).addClass("row_active");
...
function TreeGridItem (_root, _rowId, _rowIndex, _rowData){
	...
	this.getParent = function(){
		var pid = jQueryE("#" + this.id).attr("pid");
		if(pid!=""){
			var rowIndex = jQueryE("#" + pid).attr("rowIndex");
			var data = jQueryE("#" + pid).attr("data");
	...
	this.getChildren = function(){
		var arr = [];
		var trs = jQueryE(__root).find("tr[pid='" + this.id + "']");
		for(var i=0;i<trs.length;i++){
			var tr = trs[i];
