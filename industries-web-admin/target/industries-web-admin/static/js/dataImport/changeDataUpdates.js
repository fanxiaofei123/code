...
$(function () {
    document.getElementById("uploadButton").addEventListener('click', function () {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState==4) {
                ...
                if(failCount!="0" && failCount!=null){
                    message += ",点击<a href='" + basePath + "/changeValidata/downloadFailData.do'>下载失败数据</a>";
                }
        ...
        xhr.open("POST", basePath+'/changeValidata/validataButton.do', true);
...
function loadLogs(){
    $.ajax({
        url: basePath + "/changeValidata/listLogs.do",
        type: "get",
        success: function (obj) {
            ...
            for(var j=0;j<(dataList.length/6)+1;j++){
                ...
                for(var i=(j*6);i<maxJ;i++){
                    ...
                    var aLabel = "";
                    if(tmp.failCount!="0"){
                        aLabel="，点击<a href='" + basePath + "/changeValidata/downloadFailDataForLog.do?logId=" + tmp.id + "'>下载失败数据</a>";
                    }
